...
**IMPORTANT**: It’s mandatory to set a pixel-to-micrometer(μm) conversion value (μm per pixel), in order to calculate the cells/nuclei area. The input place for this value in the GUI, is named as “Length in μm”. This value is automatically filled if the corresponding metadata file (generated by the microscope after image obtaining) is present in the same folder as the image, and if it shares the same name of the image + ”_Properties.xml”.
