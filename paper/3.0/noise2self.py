...
class Cells(Dataset):
    def __init__(self, data, xy=(128,128)):
        ...
        self.cell_transforms = v2.Compose([
            ...
            v2.RandomResizedCrop(size=xy, scale=(0.75, 1.25), 
...
def train_per_image(img_noisy):
    ...
    cells_train = Cells(np.tile(img_noisy[np.newaxis,...], (8, 1, 1, 1)), xy=(128, 128))
    ...
    out = simple_output.squeeze()[ysub[0]:ysub[-1]+1, xsub[0]:xsub[-1]+1].cpu().numpy()
...
def train_test_specialist(root, n_epochs=50, lr=5e-4, test=True):
    ...
    im_train = [io.imread(Path(root / "noisy_test" / "care" / "source" / f"{i:03d}.tif"))[np.newaxis,:,:]
                for i in range(n_train)]
    im_train.extend(test_noisy)
    im_val = [io.imread(Path(root / "noisy_test" / "care" / "source" / f"{i:03d}.tif"))
    ...
    for ep in range(n_epochs):
        ...
        if ep<10 or ep%5==0 or ep==n_epochs-1:
            ...
            print(f"Loss ( {ep} ): \t train: {train_loss:.3f}, val: {val_loss:.3f}, {time.time()-tic:.2f}s")
    if not test:
        return val_loss 
    else:
