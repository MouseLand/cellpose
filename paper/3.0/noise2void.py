...
def train_per_image(img_noisy):
    ...
    print(img.shape)
    Ly, Lx = img.shape[-2:]
    patch_shape = (min(64, min(Ly//2,Lx//2)-4), min(64, min(Ly//2,Lx//2)-4))
    if patch_shape[0] < 64:
        patch_shape = (60, 60)
    ...
    im_train = np.stack((img[:Ly//2,:Lx//2], img[:Ly//2,Lx//2:2*(Lx//2)],
    ...
    config = N2VConfig(X, unet_kern_size=3, 
                    train_steps_per_epoch=X.shape[0] // train_batch_size, train_epochs=100, 
...
def train_per_image_synthetic(root, ctype="cyto2", plot=False, save=True):
    ...
    for i in trange(len(test_noisy)):
        ...
        masks = seg_model.eval(out, diameter=diam_test[i], channels=[1,0], channel_axis=0, normalize=True)[0]
...
def train_test_specialist(root, n_epochs=100, lr=5e-4, test=True):
    ...
    im_train = [io.imread(Path(root / "noisy_test" / "care" / "source" / f"{i:03d}.tif"))[np.newaxis,:,:,np.newaxis]
                for i in range(n_train)]
    im_train.extend([tn[0][np.newaxis,:,:,np.newaxis] for tn in test_noisy])
    im_val = [io.imread(Path(root / "noisy_test" / "care" / "source" / f"{i:03d}.tif"))[np.newaxis,:,:,np.newaxis]
    ...
    config = N2VConfig(X, unet_kern_size=3, 
                    ...
                    validation_steps=5,
                    train_epochs=n_epochs, train_learning_rate=lr,
                    train_loss='mse', batch_norm=True, 
                    train_batch_size=train_batch_size, n2v_perc_pix=0.198, 
                    n2v_patch_shape=patch_shape, 
                    n2v_manipulator='uniform_withCP', n2v_neighborhood_radius=5)
    ...
    if not test:
        ...
        ap, tp, fp, fn = metrics.average_precision(masks_true, masks_n2v, threshold=thresholds)
