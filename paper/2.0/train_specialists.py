...
def cluster_styles(
    train_files, train_types, train_styles, test_files, test_types, test_styles
):
    ...
    ctypes_train[
        np.array(
            [
                (train_types[i][0] == "cp") * (train_types[i][1] == "cyto")
                for i in range(len(train_types))
            ]
        ).astype(bool)
    ] = 0
    ctypes_train[
        np.array([train_types[i][0] == "lc" for i in range(len(train_types))]).astype(
            bool
        )
    ] = 1
    ctypes_train[
        np.array([train_types[i][0] == "tn" for i in range(len(train_types))]).astype(
            bool
        )
    ] = 2
    ...
    ctypes_test[
        np.array(
            [
                (test_types[i][0] == "cp") * (test_types[i][1] == "cyto")
                for i in range(len(test_types))
            ]
        ).astype(bool)
    ] = 0
    ctypes_test[
        np.array([test_types[i][0] == "lc" for i in range(len(test_types))]).astype(
            bool
        )
    ] = 1
    ctypes_test[
        np.array([test_types[i][0] == "tn" for i in range(len(test_types))]).astype(
            bool
        )
    ] = 2
    ...
    classifier = KNeighborsClassifier(n_neighbors=5).fit(
        train_styles_sub, leiden_labels
    )
...
def train_style_net(
    ...
    if train_files[0][: len(root)] != root:
        for i in range(len(train_files)):
            orig_root = os.path.split(
                os.path.split(os.path.split(train_files[i])[0])[0]
            )[0]
        ...
        for i in range(len(test_files)):
            orig_root = os.path.split(
                os.path.split(os.path.split(test_files[i])[0])[0]
            )[0]
    ...
    model = models.CellposeModel(
        pretrained_model=pretrained_model, gpu=use_gpu, diam_mean=diam_mean
    )
...
def train_general_net(
    ...
    if not style_sampling:
        ...
        ctypes[
            np.array(
                [train_types[i][0] == "lc" for i in range(len(train_types))]
            ).astype(bool)
        ] = 1
        ctypes[
            np.array(
                [train_types[i][0] == "tn" for i in range(len(train_types))]
            ).astype(bool)
        ] = 2
    ...
    optimizer = torch.optim.SGD(
        net.parameters(), lr=learning_rate, weight_decay=weight_decay, momentum=0.9
    )
    ...
    for iepoch in range(n_epochs):
        ...
        rperm = np.random.choice(
            np.arange(0, nimg), size=(nimg_per_epoch,), p=sample_probs
        )
        ...
        for k in range(0, nimg_per_epoch, batch_size):
            ...
            imgi, lbl, scale = transforms.random_rotate_and_resize(
                imgs, Y=lbls, rescale=rsc, scale_range=0.5
            )
...
def eval_model(
    model_path, test_files, test_types, use_gpu=True, flow_threshold=0.4, diameter=30.0
):

    logger_setup()
    model = models.CellposeModel(
        pretrained_model=model_path, diam_mean=diameter, gpu=use_gpu
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--style_type", default=0, type=int, help="style type to run net on"
    )
    ...
    parser.add_argument(
        "--diam_mean", default=30, type=int, help="mean diameter to resize cells to"
    )
