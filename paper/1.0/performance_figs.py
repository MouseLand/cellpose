...
def cyto(test_root, save_root, save_figure=False):
    ...
    for m, model_type in enumerate(model_types):
        masks[m].append(
            np.load(
                os.path.join(save_root, "cellpose_%s_masks.npy" % model_type),
                allow_pickle=True,
            )
        )
        masks[m].append(
            np.load(
                os.path.join(save_root, "maskrcnn_%s_masks.npy" % model_type),
                allow_pickle=True,
            )
        )
        masks[m].append(
            np.load(
                os.path.join(save_root, "stardist_%s_masks.npy" % model_type),
                allow_pickle=True,
            )
        )
        ...
        for j in range(len(masks[m])):
            aps[m].append(
                metrics.average_precision(
                    test_labels, masks[m][j], threshold=thresholds
                )[0]
            )
    ...
    for j in range(3):
        ...
        for out in outpix:
            plt.plot(out[:, 0], out[:, 1], color="y", lw=0.5)
        for out in outpix1:
            plt.plot(out[:, 0], out[:, 1], "--", color="r", lw=0.5)

        plt.title(mdl[j], color=col[j], loc="left")
        plt.text(
            0.5,
            1.05,
            "ap@0.5=%.2f" % aps[0][j][iimg, 0],
        ...
        if j == 0:
            plt.text(
                0.0,
                1.2,
                "specialist model / specialized data",
                fontsize=7,
                style="italic",
                transform=ax.transAxes,
            )
            plt.text(-0.1, 1.2, "b", fontsize=ltrf, transform=ax.transAxes)
    ...
    for j in range(3):
        ...
        if j == 0:
            plt.text(
                0.0,
                1.3,
                "generalist model / generalized data",
                fontsize=7,
                style="italic",
                transform=ax.transAxes,
            )
            plt.text(-0.1, 1.3, "c", fontsize=ltrf, transform=ax.transAxes)
    ...
    for t in range(4):
        ax = fig.add_axes([0.1 + 0.22 * t, 0.1, 0.17, 0.25])
    ...
    ax = fig.add_axes([0.05, 0.37, 0.25, 0.65])
    ...
    if save_figure:
        ...
        fig.savefig(
            os.path.join(save_root, "figs/fig_perf2d_cyto.pdf"), bbox_inches="tight"
        )
...
def nuclei(test_root, save_root, save_figure=False):
    ...
    ax = fig.add_axes([0.65, 0.3, 0.33, 0.4])
    for j in range(len(mdl)):
        ax.plot(thresholds, aps[j].mean(axis=0), color=col[j], lw=1.0)
        # print(aps[0][j][:11].mean(axis=0)[0])
    ax.spines["right"].set_visible(False)
    ax.spines["top"].set_visible(False)
    ax.set_ylim([0, 1])
    ...
    ax.text(-0.4, 1.0, "c", fontsize=ltrf, transform=ax.transAxes)
    if save_figure:
        os.makedirs(os.path.join(save_root, "figs"), exist_ok=True)
        fig.savefig(
            os.path.join(save_root, "figs/suppfig_perf2d_nuclei.pdf"),
            bbox_inches="tight",
        )
    return masks, aps
...
def suppfig_cellpose_params(test_root, save_root, save_figure=False):
    ...
    for i in range(4):  # ap_cellpose_all.shape[1]-1):
        ...
        ax.text(-0.1, 1.1, netstr[i], transform=ax.transAxes, fontsize=7)
        hb = ax.hist(
            np.clip(diffs, -1 * bmax + dbin / 2, bmax - dbin / 2),
            bins=np.arange(-bmax, bmax + dbin, dbin),
            color=colors[i],
        )
        ...
        ax.text(
            dm, max_counts * 1.1, "%0.3f" % dm + "*" * nstars, ha="center", fontsize=6
        )
        ax.set_xlabel("difference in average precision")
        if i == 0:
            ax.set_ylabel("# of test images")
        ax.text(
            -0.3, 1.1, string.ascii_lowercase[i], transform=ax.transAxes, fontsize=11
        )
        ax.spines["right"].set_visible(False)
        ax.spines["top"].set_visible(False)
    ...
    masks.append(
        np.load(
            os.path.join(save_root, "cellpose_%s_wo_sp_masks.npy" % model_type),
            allow_pickle=True,
        )
    )
    for j in range(len(masks)):
    ...
    for t in range(len(inds)):
        for j in range(len(masks)):
            ...
            if t == 0:
                ax.text(1.0, 0.7 - j * 0.18, sstr[j], color=col[j], ha="right")
            # print(aps[0][j][:11].mean(axis=0)[0])
    ax.spines["right"].set_visible(False)
    ax.spines["top"].set_visible(False)
    ax.set_ylim([0, 1])
    ...
    ax.text(
        -0.1,
        1.0,
        "remove specialized images\n       from training set",
        transform=ax.transAxes,
        fontsize=7,
    )
    ax.text(-0.3, 1.1, string.ascii_lowercase[i], transform=ax.transAxes, fontsize=11)

    if save_figure:
        os.makedirs(os.path.join(save_root, "figs"), exist_ok=True)
        fig.savefig(
            os.path.join(save_root, "figs/suppfig_cellpose_params.pdf"),
            bbox_inches="tight",
        )
...
def suppfig_metrics(test_root, save_root, save_figure=False):
    ...
    for t in range(len(inds)):
        for k in range(4):
            ax = fig.add_axes([0.1 + 0.22 * k, 0.66 - t * 0.5, 0.14, 0.28])
            ax.spines["right"].set_visible(False)
            ax.spines["top"].set_visible(False)
            if k < 3:
                for j in range(len(mdl)):
                    ax.plot(
                        scales * 30,
                        bscores[j][k][:, inds[t]].mean(axis=1),
                        color=col[j],
                    )
                ax.set_ylim([0.25, 1])
                ...
                if k == 0:
                    ax.text(
                        -0.2, 1.1, ttr[t], fontsize=7, ha="left", transform=ax.transAxes
                    )
                ...
                for j in range(len(mdl)):
                    ...
                    ax.errorbar(
                        j,
                        ajis[j][inds[t]].mean(),
                        ajis[j][inds[t]].std() / len(inds[t]) ** 0.5,
                        color="k",
                    )
                ax.text(
                    -0.2, 1.1, ttr[t], fontsize=7, ha="left", transform=ax.transAxes
    ...
    if save_figure:
        os.makedirs(os.path.join(save_root, "figs"), exist_ok=True)
        fig.savefig(
            os.path.join(save_root, "figs/suppfig_metrics.pdf"), bbox_inches="tight"
        )
...
def mask_stats(test_root, save_root, save_figure=False):
    ...
    digi = (
        np.digitize(
            np.clip(convexities.copy(), bins[0] + 0.01, bins[-1] - 0.01), bins=bins
        )
        - 1
    )
    ...
    for t in range(len(inds)):
        ax = fig.add_axes([0.1 + 0.22 * t, 0.66, 0.14, 0.28])
        for j in range(len(mdl)):
    ...
    d = np.load(
        os.path.join(save_root, "example_size_masks.npy"), allow_pickle=True
    ).item()
    msks = d["masks"]
    imgs = d["imgs"]
    inds = d["inds"]
    ...
    bs = [300, 200]
    for j in range(2):
        ax = fig.add_axes([0.07, 0.2 - 0.3 * (j), 0.14, 0.28])
    ...
    for k in range(3):
        ic = np.nonzero(
            np.logical_and(convexities < bins[k + 1], convexities > bins[k])
        )[0]
        np.random.seed(20)
        ...
        for l in range(24):
            ...
            ax.imshow(
                test_labels[im][slices[0]] == (imask + 1), vmin=0, vmax=1, cmap="gray_r"
            )
            ax.axis("off")
            if l == 0:
    ...
    dx = 0.44
    pbins = np.arange(0, 3)
    ax = fig.add_axes([0.42 + dx, 0.04, 0.11, 0.4])
    for j in range(len(masks)):
    ...
    ax.text(0.7, 1.1, "IoU threshold = 0.5", transform=ax.transAxes)
    ax.set_ylim([0, 1.0])
    ax.spines["right"].set_visible(False)
    ax.spines["top"].set_visible(False)
    # ax.set_title('generalized data', fontsize=7)
    ax.text(-0.5, 1.1, string.ascii_lowercase[s], transform=ax.transAxes, fontsize=10)
    s += 1

    ax = fig.add_axes([0.6 + dx, 0.04, 0.11, 0.4])
    for j in range(len(masks)):
        ax.errorbar(
            pbins + 0.04 * j,
            ioub[j],
