...
def get_pretrained_models(
    model_root, unet=0, nclass=3, residual=1, style=1, concatenate=0
):
    sstr = ["off", "on"]
...
def make_kfold_data(data_root):
    ...
    for j in range(9):
        ...
        test_inds = all_inds[
            np.arange(j * ntest + ntrain, (j + 1) * ntest + ntrain, 1, int) % ntot
        ]
        for i, ind in enumerate(train_inds):
            ...
            shutil.copyfile(
                flow_labels[ind], os.path.join(root_train, "%03d_img_flows.tif" % i)
            )
        for i, ind in enumerate(test_inds):
            ...
            shutil.copyfile(
                flow_labels[ind], os.path.join(root_test, "%03d_img_flows.tif" % i)
            )
...
def test_unets_main(data_root, save_root):
    ...
    for model_type in model_types:
        ...
        test_root = os.path.join(
            data_root, "images_%s/test/" % model_type.split("_")[0]
        )
...
def test_unets(model_root, test_root, save_root, model_type="cyto"):
    ...
    if model_type != "cyto_sp":
        dat = np.load(
            os.path.join(test_root, "predicted_diams.npy"), allow_pickle=True
        ).item()
        if model_type == "cyto":
            rescale = 30.0 / dat["predicted_diams"]
        else:
            rescale = 17.0 / dat["predicted_diams"]
    else:
        rescale = np.ones(len(test_data))

    for k in range(1):  # len(concatenation)):
        pretrained_models = get_pretrained_models(
            model_root, 1, nclasses[k], residual_on[k], style_on[k], concatenation[k]
        )
        print(pretrained_models)
        model = models.UnetModel(device=device, pretrained_model=pretrained_models)

        masks = model.eval(test_data, channels=channels, rescale=rescale, net_avg=True)[
            0
        ]
        ...
        for k in range(len(concatenation)):
            ...
            for l in range(4):
                ...
                if k == 0 and l == 0:
                    ...
                    ccs = np.corrcoef(
                        diams_style,
                        np.array([utils.diameters(lbl)[0] for lbl in tlabels]),
                    )[0, 1]
                    cc = np.corrcoef(
                        predicted_diams,
                        np.array([utils.diameters(lbl)[0] for lbl in tlabels]),
...
def test_timing(test_root, save_root):
    ...
    rescale = 30.0 / dat["predicted_diams"][itest]
    Ly, Lx = test_data.shape[1:]
    test_data = cv2.resize(
        np.transpose(test_data, (1, 2, 0)), (int(Lx * rescale), int(Ly * rescale))
    )
...
def test_cellpose(
    test_root, save_root, pretrained_models, diam_file=None, model_type="cyto"
):
...
def test_nets_3D(stack, model_root, save_root, test_region=None):
    ...
    cell_thresholds = [3.0, 0.25]
    boundary_thresholds = [0.0, 0.0]
    for m, model_arch in enumerate(model_archs):
        if model_arch == "cellpose":
            ...
            model = models.CellposeModel(
                device=device, pretrained_model=pretrained_models
            )
            masks = model.eval(
                stack, channels=[2, 1], rescale=30.0 / 25.0, do_3D=True, min_size=2000
            )[0]
        else:
            pretrained_models = get_pretrained_models(
                model_root,
            ...
            masks = model.eval(
                ...
                boundary_threshold=boundary_thresholds[m],
            )[0]
...
def test_cellpose_kfold_aug(data_root, save_root):
    ...
    for j in range(9):
        ...
        pretrained_models = get_pretrained_models(
            model_root, 0, 3, residual_on[k], style_on[k], concatenation[k]
        )
        print(pretrained_models)
        ...
        masks = cp_model.eval(
            test_data, channels=channels, rescale=rescale, net_avg=True, augment=True
        )[0]
        ap = metrics.average_precision(test_labels, masks, threshold=thresholds)[0]
        print(ap[:, [0, 5, 8]].mean(axis=0))
        aps[j] = ap
            ...
            if k == 0:
                ...
                for m, pretrained_model in enumerate(pretrained_models):
                    ...
                    ap = metrics.average_precision(
                        test_labels, masks, threshold=thresholds
                    )[0]
                    print(ap[:, [0, 5, 8]].mean(axis=0))
                    aps[j, m + 5] = ap
