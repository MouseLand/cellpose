...
def test_cyto2_to_seg(data_dir, image_names):
    ...
    masks, flows, styles, diams = model.eval(
        imgs, diameter=30, channels=channels, net_avg=False
    )
...
def test_class_3D(data_dir, image_names):
    ...
    for m, model_type in enumerate(model_types):
        ...
        masks = model.eval(
            img, do_3D=True, diameter=25, channels=[chan[m], chan2[m]], net_avg=False
        )[0]
...
def test_cli_2D(data_dir, image_names):
    ...
    for m, model_type in enumerate(model_types):
        cmd = (
            "python -m cellpose --dir %s --pretrained_model %s --fast_mode --chan %d --chan2 %d --diameter 0 --no_interp --save_png"
...
def test_cli_3D(data_dir, image_names):
    ...
    for m, model_type in enumerate(model_types):
        cmd = (
            "python -m cellpose --dir %s --do_3D --pretrained_model %s --fast_mode --cellprob_threshold 0 --chan %d --chan2 %d --diameter 25 --save_tif"
    ...
    for image_name in image_names:
        ...
        if check:
            if os.path.exists(output_test):
                ...
                print(
                    "average precision of [%0.3f %0.3f %0.3f]" % (ap[0], ap[1], ap[2])
                )
