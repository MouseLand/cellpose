...
def main():
    ...
    input_img_args.add_argument(
        "--dir", default=[], type=str, help="folder containing data to run or train on."
    )
    input_img_args.add_argument(
        "--look_one_level_down",
        action="store_true",
    ...
    input_img_args.add_argument(
        "--img_filter", default=[], type=str, help="end string for images to run on"
    )
    input_img_args.add_argument(
        "--channel_axis",
        default=None,
    ...
    training_args.add_argument(
        "--batch_size", default=8, type=int, help="batch size. Default: %(default)s"
    )
    training_args.add_argument(
        "--min_train_masks",
        default=5,
    ...
    training_args.add_argument(
        "--residual_on", default=1, type=int, help="use residual connections"
    )
    training_args.add_argument(
        "--style_on", default=1, type=int, help="use style vector"
    )
    training_args.add_argument(
        "--concatenation",
        default=0,
    ...
    if len(args.dir) == 0:
        ...
        if (
            args.pretrained_model is None
            or args.pretrained_model == "None"
            or args.pretrained_model == "False"
            or args.pretrained_model == "0"
        ):
        ...
        if pretrained_model and not os.path.exists(pretrained_model):
            ...
            if model_type == "nuclei":
                ...
                szmean = 30.0
        builtin_size = (
            model_type == "cyto" or model_type == "cyto2" or model_type == "nuclei"
        )
            ...
            if args.train_size:
                ...
                if test_images is not None:
                    predicted_diams, diams_style = sz_model.eval(
                        test_images, channels=None, normalize=False
                    )
                    ccs = np.corrcoef(
                        diams_style,
                        np.array([utils.diameters(lbl)[0] for lbl in test_masks]),
                    )[0, 1]
                    cc = np.corrcoef(
                        predicted_diams,
                        np.array([utils.diameters(lbl)[0] for lbl in test_masks]),
