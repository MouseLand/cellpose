...
class Cellpose:
    ...
    def __init__(self, gpu=False, model_type="cyto", net_avg=False, device=None):
        ...
        self.diam_mean = 30.0  # default for any cyto model
...
class CellposeModel(UnetModel):
    ...
    def eval(
        ...
        if isinstance(x, list) or x.squeeze().ndim == 5:
            ...
            if not model_loaded and (
                isinstance(self.pretrained_model, list) and not net_avg and not loop_run
            ):
                self.net.load_model(self.pretrained_model[0], cpu=(not self.gpu))
    ...
    def _run_cp(
        ...
        if compute_masks:
            ...
            if do_3D:
                ...
                for i in iterator:
                    outputs = dynamics.compute_masks(
                        dP[:, i],
                        cellprob[i],
                        niter=niter,
                        cellprob_threshold=cellprob_threshold,
    ...
    def loss_fn(self, lbl, y):
        ...
        veci = 5.0 * self._to_device(lbl[:, 1:])
        lbl = self._to_device(lbl[:, 0] > 0.5)
...
class SizeModel:
    ...
    def __init__(self, cp_model, device=None, pretrained_size=None, **kwargs):
        ...
        if not hasattr(self.cp, "pretrained_model"):
            error_message = (
                "no pretrained cellpose model specified, cannot compute size"
            )
