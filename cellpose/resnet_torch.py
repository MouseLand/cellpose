    ...
    def __init__(self, nbase, sz, residual_on=True, conv_3D=False):
        ...
        for n in range(len(nbase)-1):
            if residual_on:
                ...
                self.down.add_module('conv_down_%d'%n, convdown(nbase[n], nbase[n+1], sz, conv_3D))
...
class batchconvstyle(nn.Module):
    def __init__(self, in_channels, out_channels, style_channels, sz, 
        ...
        if concatenation:
            self.conv = batchconv(in_channels*2, out_channels, sz, conv_3D)
...
class resup(nn.Module):
    def __init__(self, in_channels, out_channels, style_channels, sz, 
        ...
        self.conv.add_module('conv_1', batchconvstyle(out_channels, out_channels, style_channels, sz, concatenation=concatenation, conv_3D=conv_3D))
        ...
        for n in range(1,len(nbase)):
            if residual_on:
                self.up.add_module('res_up_%d'%(n-1), 
                    resup(nbase[n], nbase[n-1], nbase[-1], sz, concatenation, conv_3D))
            else:
                self.up.add_module('conv_up_%d'%(n-1), 
                    convup(nbase[n], nbase[n-1], nbase[-1], sz, concatenation, conv_3D))
...
class CPnet(nn.Module):
    def __init__(self, nbase, nout, sz,
        ...
        self.downsample = downsample(nbase, sz, residual_on=residual_on, conv_3D=conv_3D)
        nbaseup = nbase[1:]
        nbaseup.append(nbaseup[-1])
        self.upsample = upsample(nbaseup, sz, residual_on=residual_on, 
                                 concatenation=concatenation, conv_3D=conv_3D)
