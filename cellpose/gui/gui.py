...
class MainW(QMainWindow):
    ...
    def make_buttons(self):
        ...
        for j in range(len(net_text)):
            self.StyleButtons.append(
                guiparts.ModelButton(self, net_text[j], net_text[j])
            )
        ...
        net_text = [
            ...
            "LC2",
            "LC3",
            "LC4",  #'LC-g','LC-e','LC-r','LC-n',
        ]
        nett = [
            "cellpose cyto fluorescent",
            "cellpose other",
            "tissuenet 1",
            "tissuenet 2",
            "tissuenet 3",
        ...
        self.ModelButton = QPushButton("run model")
    ...
    def model_choose(self, index):
        if index > 0:
            print(
                f"GUI_INFO: selected model {self.ModelChoose.currentText()}, loading now"
            )
    ...
    def remove_stroke(self, delete_points=True, stroke_ind=-1):
        ...
        if inZ:
            ...
            if delete_points:
                self.current_point_set = self.current_point_set[
                    : -1 * (stroke[:, -1] == 1).sum()
                ]
            self.update_layer()
    ...
    def add_mask(self, points=None, color=(100, 200, 50)):
        ...
        if points.shape[1] < 3:
            points = np.concatenate(
                (np.zeros((points.shape[0], 1), np.int32), points), axis=1
            )
    ...
    def compute_saturation(self):
        ...
        for n in iterator:
            self.saturation.append(
                [
                    np.percentile(self.stack[n].astype(np.float32), 1),
    ...
    def new_model(self):
        ...
        self.train_data, self.train_labels, self.train_files = io._get_train_set(
            image_names
        )
    ...
    def train_model(self):
        ...
        self.new_model_path = self.model.train(
            self.train_data,
            self.train_labels,
    ...
    def compute_model(self, model_name=None):
        ...
        try:
            ...
            if self.NZ > 1:
                ...
                stitch_threshold = (
                    0
                    if stitch_threshold <= 0 or stitch_threshold > 1
                    else stitch_threshold
                )
            ...
            self.flows[1] = (np.clip(normalize99(flows[2].copy()), 0, 1) * 255).astype(
                np.uint8
            )  # dist/prob
            if not do_3D and not stitch_threshold > 0:
                masks = masks[np.newaxis, ...]
                self.flows[0] = resize_image(
                    self.flows[0],
                    masks.shape[-2],
                    masks.shape[-1],
                    interpolation=cv2.INTER_NEAREST,
                )
                self.flows[1] = resize_image(
                    self.flows[1], masks.shape[-2], masks.shape[-1]
                )
            if not do_3D and not stitch_threshold > 0:
            ...
            if len(flows) > 2:
                ...
                self.flows.append(
                    np.concatenate((flows[1], flows[2][np.newaxis, ...]), axis=0)
                )  # dP, dist/prob
