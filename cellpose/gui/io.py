...
def _load_image(parent, filename=None, load_seg=True):
    ...
    if load_seg:
        if os.path.isfile(manual_file) and not parent.autoloadMasks.isChecked():
            ...
            mask_file = (
                os.path.splitext(filename)[0] + "_masks.tif"
        ...
        if image.shape[1] < 3:
            ...
            image = np.concatenate(
                (
                    image,
                    np.zeros(
                        (shape[0], 3 - shape[1], shape[2], shape[3]), dtype=np.uint8
                    ),
                ),
                axis=1,
            )
        ...
        if image.shape[-1] < 3:
            ...
            image = np.concatenate(
                (
                    image,
                    np.zeros(
                        (shape[0], shape[1], 3 - shape[2]), dtype=type(image[0, 0, 0])
                    ),
                ),
                axis=-1,
            )
        ...
        if isinstance(dat["outlines"], list):
            ...
            for k, outline in enumerate(dat["outlines"]):
                ...
                if median is not None:
                    parent.cellcolors = np.append(
                        parent.cellcolors, color[np.newaxis, :], axis=0
                    )
                    parent.ncells += 1
        else:
            if dat["masks"].ndim == 2:
    ...
    if "flows" in dat:
        ...
        try:
            if parent.flows[0].shape[-3] != dat["masks"].shape[-2]:
                ...
                parent.flows[0] = cv2.resize(
                    parent.flows[0].squeeze(), (Lx, Ly), interpolation=cv2.INTER_NEAREST
                )[np.newaxis, ...]
                parent.flows[1] = cv2.resize(
                    parent.flows[1].squeeze(), (Lx, Ly), interpolation=cv2.INTER_NEAREST
                )[np.newaxis, ...]
            if parent.NZ == 1:
...
def _masks_to_gui(parent, masks, outlines=None):
    ...
    parent.cellcolors = np.concatenate(
    ...
    if parent.NZ > 1 and parent.is_stack:
        np.save(
            base + "_seg.npy",
            {
                ...
                "flows": parent.flows,
                "zdraw": parent.zdraw,
                "model_path": parent.current_model_path
                if hasattr(parent, "current_model_path")
                else 0,
